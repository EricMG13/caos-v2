// One saved artifact, drawn the same way wherever it is read. Report and
// Committee render the identical panel over the identical v1 shape and differ
// only in the `data-` prefix their own specs select on, so the panel lives
// here rather than once per section: two copies drift, and a reader comparing
// a filing against the report behind it would be comparing two renderers.
import type { KeyboardEvent } from "react";

/** The saved-artifact shape both sections read; narrower than either document. */
...
/** Wide canonical text, kept reachable by keyboard scroll in both sections --
    Report always was; Committee's own commit (`feat: complete governed Phase 5
    workbench reads`) gives it the same scroll region for the same reason,
    rather than leaving it a plain block. `.artifact-scroll{min-height:24px}`
    (caos.css) is what keeps a one-line body at the WCAG 2.5.8 target size;
    without it, a `pre` given `tabIndex` here is an interactive control that
    reads 17px tall to the a11y layout probe. */
