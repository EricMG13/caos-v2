...
Models: follow docs/GPT_MODEL_REASONING_MATRIX.md. Use gpt-5.6-sol low for mechanical work, medium for default implementation and ordinary review, high for critical store/billing/concurrency work, and xhigh for targeted invariant audits, task acceptance and the whole-phase confidence review. Use gpt-6-astra medium for long-horizon multi-file work, high for architecture and governance, and xhigh for the separate whole-phase adversarial audit and the final all-phases review. xhigh is the ceiling; do not use max, ultra or an ultrathink prompt keyword. Record actual model, version and effort at every checkpoint.
