...
Updated at the user's request: these rules supersede the earlier blanket requirement to tournament every non-trivial function and any per-PR/per-edit adversarial-audit cadence for this implementation. Ordinary tests, static/security checks and `confidence-review` after code changes remain required.
...
| `rewrite-tournament` | **Only changed, non-trivial high-risk sections**, after implementing and testing their fix, before declaring those changes done. No whole-codebase tournament | **`max` reasoning** for each tournament role; preserve caller contracts/invariants, test the winner and report it. “Incumbent holds” is valid |
| `adversarial-reviewer` code audit | **Phase completion only**, after the phase's implementation, focused tournaments and normal checks. Audit the full phase diff plus affected callers, not just the final PR | **`max` reasoning**; Saboteur, New Hire and Security Auditor passes, deduplicated evidence-backed findings and a phase verdict |
...
Skip tournaments for low-risk presentation/layout, straightforward plumbing, mechanical changes, documentation/configuration/test-only edits, generated files and vendor files. Record the reason. Scope each tournament pass to at most the two most material qualifying symbols; use additional bounded passes only for remaining changed high-risk symbols. Build the impact set with GitNexus symbol context and upstream caller impact, then verify actual source references/types/tests—an index is discovery evidence, not a correctness oracle.

Configure the actual executor's reasoning setting to `max` for tournament roles and the phase auditor; do not merely put “think harder” in their prompts or silently substitute `high`/`xhigh`. Keep the configured model unless the user requests another. If the execution environment cannot provide `max`, report that limitation before the affected review gate is considered satisfied.
...
Phase-close order: implementation → normal tests and confidence review → qualifying high-risk tournaments (`max`) → rerun affected checks → refresh GitNexus → end-of-phase adversarial code audit (`max`) → remediate/reverify findings → phase accepted. If remediation changes the indexed code, refresh the index again before recording the final phase identity. This cadence does not disable automated security scans or ordinary PR review.
...
6. Prove the catalog-selected LITE earnings route `CP-0 → CPL-10 → CP-5`; CP-5 must remain its full QA workflow over the named LITE object, not a shortened QA prompt. Keep other routes disabled until equivalent contract tests exist.
...
| Review/diff | `confidence-review` after code changes; **`rewrite-tournament` only for changed high-risk non-trivial sections at `max` reasoning**; ordinary PR review; existing 800-counted-line PR ceiling | Record risk-based scope/skips, test every proposed winner, fix root causes, and split oversized concerns |
| Phase-completion audit | **`adversarial-reviewer` only at phase completion, at `max` reasoning**, over the full phase diff and affected callers | No intermediate/per-PR adversarial audit requirement; confirmed blockers must be fixed and reverified before accepting the phase |
...
4. `confidence-review` has run after code changes. Changed high-risk non-trivial sections have completed `rewrite-tournament` at `max` reasoning; low-risk/trivial/doc/config/test/generated/vendor skips are recorded. Confirmed new issues are patched and retested. An individual repair does not trigger an adversarial audit: the full `max`-reasoning adversarial code audit runs only at phase completion, as specified in section 5.
