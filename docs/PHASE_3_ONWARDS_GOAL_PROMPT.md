...
Models: follow docs/GPT_MODEL_REASONING_MATRIX.md. Use gpt-5.6-sol low for mechanical work, medium for default implementation and ordinary review, and xhigh for targeted invariant audits, confidence and task acceptance. Use gpt-6-astra high for plans and architecture and xhigh for the independent whole-phase adversarial review. xhigh is the ceiling; do not use max, ultra or an ultrathink prompt keyword. Verify and record model/effort. Add no speculative scope.
...
Use TDD and ordinary review per task. After committing and remediation, pass the 800-line gate against the actual PR base. At each phase end run full local/required CI gates, one confidence-review on gpt-5.6-sol xhigh, remediate/retest, then one adversarial audit on gpt-6-astra xhigh. Revalidate changed candidate evidence, including qualification. Record acceptance in the tracked handoff, then continue directly through the next phase until Phases 3–6 are complete. Pause only for a failed gate, missing explicit authorization (including spend, hosted writes or deployment), or a genuine blocker.
