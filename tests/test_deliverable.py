...
def test_filing_before_a_freeze_is_refused(case: tuple[StoreConnection, UUID]) -> None:
    """Signed is not frozen, and neither is nothing. A filer needs both a
    signature and a freeze on record, not just standing to attempt it."""
    conn, case_id = case
    actor = _approver(conn, case_id)

    with pytest.raises(Refusal) as caught:
        file_deliverable(conn, case_id=case_id, actor_id=actor, revision_id=REVISION)
